...
// make it possible to reset the config/config_inc.php file if it's already filled with data
if( $onlyDuringFirstInstall ) {
...
if( !empty( $gBitDbType ) && $gBitInstaller->isPackageInstalled('users') && !$gBitUser->isAdmin() && !$_SESSION['first_install'] ) {
	$install_file = 'login';
